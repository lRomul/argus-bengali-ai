...
def train_fold(save_dir, train_folds, val_folds):
    ...
    for image_size, batch_size, ekush_prob, epochs in zip(IMAGE_SIZE, BATCH_SIZE, EKUSH_PROB, TRAIN_EPOCHS):
        ...
        ekush_transform = get_transforms(train=True, size=image_size, gridmask_p=0.0, pad=12)
        mixer = CutMix(num_mix=1, beta=1.0, prob=1.0)
        test_transform = get_transforms(train=False, size=image_size)

        ekush_dataset = EkushDataset(ekush_data, train_folds, transform=ekush_transform, mixer=mixer)
        val_dataset = EkushDataset(ekush_data, val_folds, transform=test_transform)

        train_loader = DataLoader(ekush_dataset, batch_size=batch_size,
        ...
        callbacks = [
            MonitorCheckpoint(save_dir, monitor='val_ekush_recall', max_saves=1, better='max'),
            EarlyStopping(monitor='val_ekush_recall', patience=30, better='max'),
            ReduceLROnPlateau(monitor='val_ekush_recall', better='max',
